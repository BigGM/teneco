<?php
  ...
  $uploadPath = $home_docs_path . "/" . $docName;

  $ris = move_uploaded_file( $tempPath, $uploadPath );

  if (!$ris) {
      echo 'Exception: File video non trasferito';
      die();
  }

  // NB. url da inserire in ambiente di esercizio
  
  //$url = "http://".$_SERVER['SERVER_NAME'].":".$_SERVER['SERVER_PORT']."/GCA/video/". $docName;
  $url = "/GCA/video/". $docName;
