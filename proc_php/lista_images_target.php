 ...
 **/
$cmd  = 'BEGIN ' . $proc . '(:categoria, :outcome, :cursor); END;';
...
oci_bind_by_name($stmt, ':categoria' , $categoria, 200);
...
{
    $myObj = new stdClass;
    $myObj->id        = rawurlencode( $row[0] );
    $myObj->url       = rawurlencode( $row[1] );
    $myObj->nome      = rawurlencode( $row[2] );
    $myObj->categoria = rawurlencode( $row[4] );         // [4] e' corretto, perche' la procedura da' un campo [3] che non mi interessa
    $myObj->descr     = rawurlencode( $row[5] );
